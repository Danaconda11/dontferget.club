...
export default class TodoList extends Component {
  ...
  on_drop_hover(index, todo) {
    this.setState({hover: {index, todo}})
  }
  on_drop(...args) {
    ...
    return (
      <div className="todo_list">
        <header className="d-flex">
          ...
          <i
            className="settings_icon fa fa-gear"
            onClick={() => this.setState({view_settings: true})}
          />
          <div className="ml-auto">
            <Link
              className={
                'btn btn-sm ' + (showDone ? 'btn-link' : 'btn-primary')
              }
              to={`/list/${list.name}`}
            >
              Doing
        ...
        >
          ...
          <div className="d-flex ai-center">
            <Toggle
              ...
              onChange={this.on_sharing_update}
            />
            <label className="mr-1" htmlFor="public_sharing">
              Public
            </label>
            {sharing.public && (
              <CopyUrl>{`https://dontferget.club/list/${list.uuid}`}</CopyUrl>
            )}
          </div>
          <div className="d-flex ai-center">
            <Toggle
              ...
              onChange={this.on_sharing_update}
            />
            <label className="mr-1" htmlFor="private_sharing">
              Private
            </label>
            {sharing.private && (
              <CopyUrl>{`https://dontferget.club/list/${
                sharing.private
              }`}</CopyUrl>
            )}
          </div>
